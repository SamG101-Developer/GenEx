...
auto do_position(I &&first, S &&last, Pred &&pred, Proj &&proj = {}, const genex::algorithms::pos_t def = -1) -> genex::algorithms::pos_t {
    genex::algorithms::pos_t index = 0;
...
auto do_position(Rng &&rng, Pred &&pred, Proj &&proj = {}, const genex::algorithms::pos_t def = -1) -> genex::algorithms::pos_t {
    return do_position(genex::iterators::begin(rng), genex::iterators::end(rng), std::forward<Pred>(pred), std::forward<Proj>(proj), def);
}


template <iterator I, sentinel S, std::invocable<iter_value_t<I>> Proj = genex::meta::identity, std::predicate<std::invoke_result_t<Proj, iter_value_t<I>>> Pred>
auto do_position_last(I &&first, S &&last, Pred &&pred, Proj &&proj = {}, const genex::algorithms::pos_t def = -1) -> genex::algorithms::pos_t {
    genex::algorithms::pos_t index = genex::iterators::distance(first, last) - 1;
...
template <range Rng, std::invocable<range_value_t<Rng>> Proj = genex::meta::identity, std::predicate<std::invoke_result_t<Proj, range_value_t<Rng>>> Pred>
auto do_position_last(Rng &&rng, Pred &&pred, Proj &&proj = {}, const genex::algorithms::pos_t def = -1) -> genex::algorithms::pos_t {
    return do_position_last(genex::iterators::begin(rng), genex::iterators::end(rng), std::forward<Pred>(pred), std::forward<Proj>(proj), def);
}
...
namespace genex::algorithms {
    struct position_fn final : detail::algorithm_base {
        ...
        constexpr auto operator()(I &&first, S &&last, Pred &&pred, Proj &&proj = {}, pos_t def = -1) const -> std::size_t {
            MAP_TO_IMPL(do_position, first, last, pred, proj, def);
        }
        ...
        constexpr auto operator()(Rng &&rng, Pred &&pred, Proj &&proj = {}, pos_t def = -1) const -> std::size_t {
            MAP_TO_IMPL(do_position, rng, pred, proj, def);
        }
    ...
    struct position_last_fn final : detail::algorithm_base {
        template <iterator I, sentinel S, std::invocable<iter_value_t<I>> Proj = meta::identity, std::predicate<std::invoke_result_t<Proj, iter_value_t<I>>> Pred>
        constexpr auto operator()(I &&first, S &&last, Pred &&pred, Proj &&proj = {}, pos_t def = -1) const -> std::size_t {
            MAP_TO_IMPL(do_position_last, first, last, pred, proj, def);
        }

        template <range Rng, std::invocable<range_value_t<Rng>> Proj = meta::identity, std::predicate<std::invoke_result_t<Proj, range_value_t<Rng>>> Pred>
        constexpr auto operator()(Rng &&rng, Pred &&pred, Proj &&proj = {}, pos_t def = -1) const -> std::size_t {
            MAP_TO_IMPL(do_position_last, rng, pred, proj, def);
        }
