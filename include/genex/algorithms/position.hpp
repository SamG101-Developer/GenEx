...
namespace genex::algorithms {
    DEFINE_ALGORITHM(position) {
        ...
        auto operator()(Pred &&pred, Proj &&proj = {}) const -> auto {
            return
                [FWD_CAPTURES(pred, proj)]<typename Rng> requires concepts::can_position_range<Rng, Pred, Proj>
    ...
    DEFINE_ALGORITHM(position_last) {
        ...
        auto operator()(Pred &&pred, Proj &&proj = {}) const -> auto {
            return
                [FWD_CAPTURES(pred, proj)]<typename Rng> requires concepts::can_position_last_range<Rng, Pred, Proj>
                (Rng &&rng) mutable -> auto {
                return (*this)(std::forward<Rng>(rng), std::forward<Pred>(pred), std::forward<Proj>(proj));
            };
        }
