...
namespace genex::algorithms {
    ...
    DEFINE_ALGORITHM(any_of) {
        template <typename I, typename S, typename Pred, typename Proj = meta::identity> requires can_any_of_iters<I, S, Pred, Proj>
        ...
        template <typename Rng, typename Pred, typename Proj = meta::identity> requires can_any_of_range<Rng, Pred, Proj>
        ...
        constexpr auto operator()(Pred &&pred, Proj &&proj = {}) const -> auto {
            return
                [FWD_CAPTURES(pred, proj)]<typename Rng> requires can_any_of_range<Rng, Pred, Proj>
