...
namespace genex::algorithms {
    ...
    template <typename Rng, typename E, typename C, typename Proj>
    concept can_binary_search_range =
        forward_range<Rng> and
        std::indirect_strict_weak_order<C, std::projected<iterator_t<Rng>, Proj>, const E*>;

    DEFINE_ALGORITHM(binary_search) {
        template <typename I, typename S, typename E, typename Comp = operations::lt, typename Proj = meta::identity> requires can_binary_search_iters<I, S, E, Comp, Proj>
        constexpr auto operator()(I first, S last, E &&elem, Comp &&comp = {}, Proj &&proj = {}) const -> bool {
        ...
        template <typename Rng, typename E, typename Comp = operations::lt, typename Proj = meta::identity> requires can_binary_search_range<Rng, E, Comp, Proj>
        constexpr auto operator()(Rng &&rng, E &&elem, Comp &&comp = {}, Proj &&proj = {}) const -> bool {
        ...
        constexpr auto operator()(E &&elem, Comp &&comp = {}, Proj &&proj = {}) const -> auto {
            return
                [FWD_CAPTURES(elem, comp, proj)]<typename Rng> requires can_binary_search_range<Rng, E, Comp, Proj>
