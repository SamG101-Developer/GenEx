...
auto do_ptr_unique(I &&first, S &&last) -> genex::generator<typename deref_value_t<I>::pointer> {
...
auto do_ptr_unique(Rng &&rng) -> genex::generator<typename range_value_t<Rng>::pointer> {
    for (auto &&x : rng) {
        co_yield x.get();
    }
}
...
namespace genex::views {
    struct ptr_unique_fn final : detail::view_base {
        ...
        constexpr auto operator()(I &&first, S &&last) const -> decltype(auto) {
            MAP_TO_IMPL(do_ptr_unique, first, last);
        }

        template <range Rng> requires (unique_ptr<range_value_t<Rng>>)
        constexpr auto operator()(Rng &&rng) const -> decltype(auto) {
            MAP_TO_IMPL(do_ptr_unique, rng);
        }
