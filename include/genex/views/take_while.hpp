...
namespace genex::views::detail {
    ...
    struct take_while_iterator {
        ...
        GENEX_INLINE constexpr auto operator++(int) noexcept(
            noexcept(it++)) -> take_while_iterator {
            auto tmp = *this;
            ++*this;
            return tmp;
        }
    ...
    template <typename I, typename S, typename Pred, typename Proj>
    requires concepts::takeable_while_iters<I, S, Pred, Proj>
    take_while_iterator(I, S, Pred, Proj) -> take_while_iterator<I, S, Pred, Proj>;

    template <typename I, typename S, typename Pred, typename Proj>
    requires concepts::takeable_while_iters<I, S, Pred, Proj>
    GENEX_VIEW_ITERSENT_EQOP_DEFINITIONS(take_while_iterator, take_while_sentinel, I, S, Pred, Proj) {
        return it.it == it.st;
    }
    ...
    struct take_while_view : std::ranges::view_interface<take_while_view<V, Pred, Proj>> {
        ...
        GENEX_INLINE constexpr explicit take_while_view() noexcept = default;
        ...
        GENEX_INLINE constexpr auto internal_end() const noexcept(
            ...
            noexcept(iterators::end(base_rng)) and
            std::is_nothrow_invocable_v<Pred&, std::invoke_result_t<Proj&, iter_reference_t<iterator_t<V>>>>) {
