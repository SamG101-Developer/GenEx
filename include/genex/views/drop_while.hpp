...
namespace genex::views::detail {
    ...
    struct drop_while_iterator {
        ...
        GENEX_VIEW_ITERATOR_CTOR_DEFINITIONS(drop_while_iterator);
        GENEX_VIEW_ITERATOR_FUNC_DEFINITIONS(drop_while_iterator);

        GENEX_INLINE constexpr explicit drop_while_iterator(I it, S st) noexcept(
    ...
    struct drop_while_sentinel {
        GENEX_VIEW_SENTINEL_CTOR_DEFINITIONS(drop_while_sentinel);
        GENEX_VIEW_SENTINEL_FUNC_DEFINITIONS(drop_while_iterator, drop_while_sentinel, Pred, Proj);
    };
    ...
    struct drop_while_view : std::ranges::view_interface<drop_while_view<V, Pred, Proj>> {
        GENEX_VIEW_VIEW_CTOR_DEFINITIONS(drop_while_view);
        GENEX_VIEW_VIEW_TYPE_DEFINITIONS(drop_while_iterator, drop_while_sentinel, Pred, Proj);
        ...
        GENEX_INLINE constexpr auto internal_begin() const noexcept(noexcept(iterators::begin(base_rng)) and std::is_nothrow_invocable_v<Pred&, std::invoke_result_t<Proj&, range_reference_t<V>>>) {
...
namespace genex::views {
    struct drop_while_fn {
        ...
        GENEX_INLINE constexpr auto operator()(Rng&& rng, Pred pred, Proj proj = {}) const -> auto {
            return detail::drop_while_view<std::views::all_t<Rng>, Pred, Proj>{
