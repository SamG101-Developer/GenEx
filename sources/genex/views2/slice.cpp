...
namespace genex::views {
  struct slice_fn {
    ...
    GENEX_INLINE constexpr auto operator()(I first, S last, const Int n, const Int m) const noexcept(
      ...
      GENEX_IGNORE(last);
      return genex::span<iter_value_t<I>>(std::move(first) + static_cast<std::ptrdiff_t>(n),
                                          std::move(first) + static_cast<std::ptrdiff_t>(m));
    }
    ...
    GENEX_INLINE constexpr auto operator()(I first, S last, const Int n, const Int m) const noexcept(
      ...
      return std::ranges::subrange(
        std::counted_iterator(iterators::next(std::move(first), static_cast<iter_difference_t<I>>(n)),
                              static_cast<iter_difference_t<I>>(m - n)),
    ...
    GENEX_INLINE constexpr auto operator()(Rng &&rng, const Int n, const Int m) const noexcept(
      ...
      return genex::span<range_value_t<Rng>>(std::move(first) + static_cast<std::ptrdiff_t>(n),
                                             std::move(first) + static_cast<std::ptrdiff_t>(m));
    }
    ...
    GENEX_INLINE constexpr auto operator()(Rng &&rng, const Int n, const Int m) const noexcept(
      ...
      auto [first, last] = iterators::iter_pair(rng);
      return std::ranges::subrange(
        std::counted_iterator(iterators::next(std::move(first), static_cast<iter_difference_t<iterator_t<Rng>>>(n)),
                              static_cast<iter_difference_t<iterator_t<Rng>>>(m - n)),
