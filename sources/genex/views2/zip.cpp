...
namespace genex::views::detail::concepts {
  ...
  struct zippable_iters_helper<std::tuple < Is...>
  ,
  std::tuple<Ss...>
  >
 {
  ...
  zippable_iters<std::tuple < iterator_t<Rngs>...>
  ,
  std::tuple<sentinel_t<Rngs>...>
  >;
}
...
namespace genex::views::detail::impl {
  ...
  struct zip_iterator {
    ...
    std::conditional_t<
      (std::random_access_iterator<iterator_t<Rngs>> and...
    )
    ,
    std::random_access_iterator_tag
    ,
    std::conditional_t<
      (std::bidirectional_iterator<iterator_t<Rngs>> and...
    )
    ,
    std::bidirectional_iterator_tag
    ,
    std::conditional_t<
      (std::forward_iterator<iterator_t<Rngs>> and...
    )
    ,
    ...
    )
 {
    ...
    template <typename Self>
    GENEX_INLINE constexpr auto operator--(this Self &&self, int) -> zip_iterator requires (bidirectional_range<Rngs>
                                and
    ...
    )
 {
    ...
    template <typename Self>
    GENEX_INLINE constexpr auto operator+=(this Self &&self, difference_type n) -> zip_iterator& requires (
                                  random_access_range<Rngs> and
    ...
    )
 {
    ...
    )
 {
    ...
    template <typename Self>
    GENEX_INLINE constexpr auto operator+(this Self &&self, zip_iterator const &that) -> difference_type requires (
                                  random_access_range<Rngs> and
    ...
    )
 {
    ...
    )
 {
  ...
  private:
    ...
    constexpr auto any_iterator_finished_impl(
      std::tuple<iterator_t<Rngs>...> const &its, std::tuple<sentinel_t<Rngs>...> const &sts,
      std::index_sequence<Is...>) -> bool {
      return (...
      or(std::get < Is > (its) == std::get < Is > (sts)));
    }
...
namespace genex::views {
  struct zip_fn {
    template <typename... Is, typename... Ss>
      requires detail::concepts::zippable_iters<std::tuple < Is...>
    ...
    )
 {
    ...
    template <typename... Rngs>
      requires detail::concepts::zippable_range<Rngs...>
    and (
    sizeof
    ...
    (Rngs)
    ...
    1
      ...
      SAFE_MOVE(std::tuple < iterator_t<Rngs>...>) and SAFE_MOVE
    (std::tuple<sentinel_t<Rngs>...>)
    )
 {
