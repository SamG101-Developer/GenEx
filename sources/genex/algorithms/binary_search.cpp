...
namespace genex::algorithms::detail::impl {
  ...
  GENEX_INLINE constexpr auto do_binary_search(I first, S last, E &&elem, Comp &&comp, Proj &&proj) -> bool {
    ...
    return first != last
      and not meta::invoke(comp, elem, meta::invoke(proj, *first));
  }
}
...
namespace genex {
  struct binary_search_fn {
    ...
    GENEX_INLINE constexpr auto operator()(I first, S last, E &&elem, Comp &&comp = {},
      Proj &&proj = {}) const -> bool {
      return algorithms::detail::impl::do_binary_search(
        std::move(first), std::move(last), std::forward<E>(elem), std::forward<Comp>(comp), std::forward<Proj>(proj));
    }
