...
namespace genex::algorithms::detail::concepts {
  ...
  concept left_foldable_iters =
    ...
    std::invocable<F, Acc, iter_reference_t<I>> and
    std::convertible_to<std::invoke_result_t<F, Acc, iter_reference_t<I>>, Acc>;
...
namespace genex::algorithms::detail::impl {
  ...
  GENEX_INLINE constexpr auto do_fold_left(I first, S last, E &&init, F &&f) -> std::remove_cvref_t<E> {
...
namespace genex {
  struct fold_left_fn {
    ...
    GENEX_INLINE constexpr auto operator()(I first, S last, E &&init, F &&f) const {
      return algorithms::detail::impl::do_fold_left(
