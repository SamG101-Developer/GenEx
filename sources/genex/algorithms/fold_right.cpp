...
namespace genex::algorithms::detail::concepts {
  template <typename I, typename S, typename E, typename F, typename Acc = std::remove_cvref_t<E>>
  concept right_foldable_iters =
    std::bidirectional_iterator<I> and
    std::sentinel_for<S, I> and
    std::movable<Acc> and
    std::constructible_from<Acc, E> and
    std::invocable<F, iter_reference_t<I>, Acc> and
    std::convertible_to<std::invoke_result_t<F, iter_reference_t<I>, Acc>, Acc>;

  template <typename Rng, typename E, typename F>
  concept right_foldable_range =
...
namespace genex::algorithms::detail::impl {
  ...
  GENEX_INLINE constexpr auto do_fold_right(I first, S last, E &&init, F &&f) -> std::remove_cvref_t<E> {
    auto acc = std::remove_cvref_t<E>(std::forward<E>(init));
    while (first != last) {
      --last;
...
namespace genex {
  struct fold_right_fn {
    ...
    GENEX_INLINE constexpr auto operator()(I first, S last, E &&init, F &&f) const {
      return algorithms::detail::impl::do_fold_right(
        std::move(first), std::move(last), std::forward<E>(init), std::forward<F>(f));
    }

    template <typename Rng, typename E, typename F>
      requires algorithms::detail::concepts::right_foldable_range<Rng, E, F>
