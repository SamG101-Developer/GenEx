...
namespace genex::algorithms::detail::impl {
  template <typename I, typename S, typename Pred, typename Proj, typename Int>
    ...
    and std::bidirectional_iterator<I>
  ...
  GENEX_INLINE constexpr auto do_position_last(I first, S last, Pred &&pred, Proj &&proj, const Int def) -> Int {
    auto pos = def;
    for (auto i = std::ptrdiff_t{0}; first != last; ++first, ++i) {
      if (meta::invoke(pred, meta::invoke(proj, *first))) { pos = static_cast<Int>(i); }
    }
    return pos;
  }
}
...
namespace genex {
  struct position_last_fn {
    ...
    GENEX_INLINE constexpr auto operator()(I first, S last, Pred &&pred, Proj &&proj = {},
      const Int def = -1z) const -> Int {
      return algorithms::detail::impl::do_position_last(
    ...
    GENEX_INLINE constexpr auto operator()(Rng &&rng, Pred &&pred, Proj &&proj = {},
      const Int def = -1z) const -> Int {
