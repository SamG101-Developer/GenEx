...
export module genex.algorithms.equals;
...
namespace genex::algorithms::detail::impl {
  template <typename I1, typename S1, typename I2, typename S2, typename Comp, typename Proj1, typename Proj2>
    ...
    GENEX_INLINE constexpr auto do_equals(I1 first1, S1 last1, I2 first2, S2 last2, Comp &&comp, Proj1 &&proj1,
    Proj2 &&proj2) -> bool {
    for (; first1 != last1 and first2 != last2;
    ++first1, ++first2)
    {
...
namespace genex {
  struct equals_fn {
    ...
    template <typename Rng1, typename Rng2, typename Comp = operations::eq, typename Proj1 = meta::identity, typename
      ...
      return algorithms::detail::impl::do_equals(std::move(first1), std::move(last1), std::move(first2),
                                                 std::move(last2), std::forward<Comp>(comp), std::forward<Proj1>(proj1),
                                                 std::forward<Proj2>(proj2));
    }
  };

  export inline constexpr equals_fn equals{};
}
