...
namespace genex::algorithms::detail::impl {
  ...
  GENEX_INLINE constexpr auto do_position(I first, S last, Pred &&pred, Proj &&proj, const Int def,
    const std::ptrdiff_t drop) -> Int {
...
namespace genex {
  struct position_fn {
    template <typename I, typename S, typename Pred, typename Proj = meta::identity, typename Int = std::ptrdiff_t>
      requires algorithms::detail::concepts::positionable_iters<I, S, Pred, Proj, Int>
    GENEX_INLINE constexpr auto operator()(I first, S last, Pred &&pred, Proj &&proj = {}, const Int def = -1z,
      const std::ptrdiff_t drop = 0z) const -> Int {
      return algorithms::detail::impl::do_position(
    ...
    GENEX_INLINE constexpr auto operator()(Rng &&rng, Pred &&pred, Proj &&proj = {}, const Int def = -1z,
      const std::ptrdiff_t drop = 0z) const -> Int {
