...
namespace genex::views {
    struct zip_fn {
        ...
        GENEX_INLINE constexpr auto operator()(Rngs &&... ranges) const {
            return detail::do_zip(std::make_tuple(iterators::begin(std::forward<Rngs>(ranges))...), std::make_tuple(iterators::end(std::forward<Rngs>(ranges))...));
        }
