...
namespace genex::views::detail::impl {
    ...
    struct concat_iterator {
        ...
        template <typename Self>
        GENEX_INLINE constexpr auto operator--(this Self &&self) -> concat_iterator& requires (bidirectional_range<Rngs> && ...) {
            self.template dec_impl<sizeof...(Rngs) - 1>();
            return self;
        }

        template <typename Self>
        GENEX_INLINE constexpr auto operator--(this Self &&self, int) -> concat_iterator requires (bidirectional_range<Rngs> && ...) {
            auto temp = self;
            --self;
            return temp;
        }
        ...
        template <typename Self>
        GENEX_INLINE constexpr auto operator+=(this Self &&self, difference_type n) -> concat_iterator& requires (random_access_range<Rngs> && ...) {
            self.add(n);
            return self;
        }
        ...
        template <typename Self>
        GENEX_INLINE constexpr auto operator+(this Self &&self, concat_iterator const &that) -> difference_type requires (random_access_range<Rngs> && ...) {
    ...
    private:
        ...
        template <typename Self>
        GENEX_INLINE constexpr auto add(this Self &&self, difference_type n) -> void requires (random_access_range<Rngs> && ...) {
            self.template add_impl<0>(n);
        }

        template <std::size_t I, typename Self>
        GENEX_INLINE constexpr auto add_impl(this Self &&self, difference_type n) -> void requires (random_access_range<Rngs> && ...) {
        ...
        template <typename Self>
        GENEX_INLINE constexpr auto sub(this Self &&self, difference_type n) -> void requires (random_access_range<Rngs> && ...) {
            self.template sub_impl<sizeof...(Rngs) - 1>(n);
        }

        template <std::size_t I, typename Self>
        GENEX_INLINE constexpr auto sub_impl(this Self &&self, difference_type n) -> void requires (random_access_range<Rngs> && ...) {
        ...
        GENEX_INLINE static constexpr auto dist(concat_iterator const &a, concat_iterator const &b) -> difference_type requires (random_access_range<Rngs> && ...) {
            return concat_iterator::dist_impl<0>(a, b);
        }

        template <std::size_t I>
        GENEX_INLINE static constexpr auto dist_impl(concat_iterator const &a, concat_iterator const &b) -> difference_type requires (random_access_range<Rngs> && ...) {
    ...
    private:
        template <std::size_t I, typename Self>
        GENEX_INLINE constexpr auto size_impl(this Self &&self) -> std::size_t requires (sized_range<Rngs> && ...) {
            if constexpr (I >= sizeof...(Rngs)) { return 0; }
            else {
                auto const &it = genex::get<I>(self.its);
                auto const &st = genex::get<I>(self.sts);
