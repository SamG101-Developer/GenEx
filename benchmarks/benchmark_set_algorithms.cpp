...
static void BM_Genex_V2_SetSortedDifference(benchmark::State &state) {
    ...
    for (auto _ : state) {
        auto result = data1 | genex::views::set_difference(data2) | genex::to<std::vector>();
        benchmark::DoNotOptimize(result);
    }
}
...
static void BM_Genex_V2_SetSortedUnion(benchmark::State &state) {
    ...
    for (auto _ : state) {
        auto result = data1 | genex::views::set_union(data2);
...
static void BM_Genex_V2_SetSortedIntersection(benchmark::State &state) {
    ...
    for (auto _ : state) {
        auto result = data1 | genex::views::set_intersection(data2) | genex::to<std::vector>();
        benchmark::DoNotOptimize(result);
    }
}
...
static void BM_Genex_V2_SetSortedSymmetricDifference(benchmark::State &state) {
    ...
    for (auto _ : state) {
        auto result = data1 | genex::views::set_symmetric_difference(data2) | genex::to<std::vector>();
        benchmark::DoNotOptimize(result);
    }
}
