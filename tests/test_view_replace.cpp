...
TEST(GenexViewsReplace, VecInput) {
    ...
    genex::views2::replace(5, 9)(vec);

    const auto rng = vec
        | genex::views2::replace(5, 9)
        | genex::to<std::vector>();
...
TEST(GenexViewsReplaceIf, VecInput) {
    ...
    const auto rng = vec
        | genex::views2::replace_if([](auto x) { return x > 5; }, 9)
        | genex::to<std::vector>();
...
TEST(GenexViewsReplaceIf, VecInputMultiple) {
    ...
    const auto rng = vec
        | genex::views2::replace_if([](auto x) { return x > 5; }, 9)
        | genex::views2::replace_if([](auto x) { return x < 3; }, 9)
        | genex::to<std::vector>();
