...
TEST(GenexViewsTransform, VecInputStruct) {
  ...
  const auto rng = vec
    ...
    | genex::to<std::vector>();
  const auto exp = std::vector{
    TestStruct{"one!!!", 2}, TestStruct{"two!!!", 4}, TestStruct{"three!!!", 6}, TestStruct{"four!!!", 8},
    TestStruct{"five!!!", 10}, TestStruct{"six!!!", 12}
  };
  EXPECT_EQ(rng, exp);
}
...
TEST(GenexViewsTransform, Stacked) {
  ...
  const auto rng = genex::views::zip(x, y)
    ...
    | genex::views::transform([](const auto &pair) {
      return std::make_tuple(genex::get<0>(pair) * 2, genex::get<1>(pair) * 2);
    })
    | genex::views::transform([](const auto &pair) {
      return std::make_tuple(genex::get<0>(pair) * 2, genex::get<1>(pair) * 2);
    })
