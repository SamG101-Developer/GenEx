...
TEST(GenexViewsZip, VecInputTransform) {
  ...
  const auto rng = genex::views::zip(vec1 | genex::views::transform(transform),
                                     vec2 | genex::views::transform(transform), vec3)
    | genex::to<std::vector>();
  const auto exp = std::vector<std::tuple<int, int, int>>{
    {0, 10, 10}, {2, 12, 11}, {4, 14, 12}, {6, 16, 13}, {8, 18, 14}
  };
