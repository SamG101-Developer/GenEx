...
import genex.algorithms.find_last;
import genex.algorithms.find_last_if;
...
TEST(GenexAlgosFind, FindElementExists) {
    ...
    const auto it = genex::find(vec, 4);
    EXPECT_EQ(*it, 4);
...
TEST(GenexAlgosFind, FindElementNotExists) {
    auto vec = std::vector{1, 2, 3, 4, 5, 6};
    const auto it = genex::find(vec, 10);
...
TEST(GenexAlgosFindLast, FindLastElementExists) {
    ...
    const auto it = genex::find_last(vec, 4);
    EXPECT_EQ(*it, 4);
...
TEST(GenexAlgosFindLast, FindLastElementNotExists) {
    auto vec = std::vector{1, 2, 3, 4, 5, 6};
    const auto it = genex::find_last(vec, 10);
    EXPECT_EQ(it, vec.end());
}
...
TEST(GenexAlgosFindIf, FindIfElementExists) {
    auto vec = std::vector{1, 2, 3, 4, 5, 6};
    const auto it = genex::find_if(vec, [](const int v) { return v % 2 == 0; });
    EXPECT_EQ(*it, 2);
...
TEST(GenexAlgosFindLastIf, FindLastIfElementExists) {
    auto vec = std::vector{1, 2, 3, 4, 5, 6, 4};
    const auto it = genex::find_last_if(vec, [](const int v) { return v % 2 == 0; });
    EXPECT_EQ(*it, 4);
    EXPECT_EQ(genex::iterators::distance(vec.begin(), it), 6);
}
...
TEST(GenexAlgosFindIfNot, FindIfNotElementExists) {
    auto vec = std::vector{1, 2, 3, 4, 5, 6};
    const auto it = genex::find_if_not(vec, [](const int v) { return v % 2 == 0; });
    EXPECT_EQ(*it, 1);
...
TEST(GenexAlgosFindLastIfNot, FindLastIfNotElementExists) {
    ...
    const auto it = genex::find_last_if_not(vec, [](const int v) { return v % 2 == 0; });
    EXPECT_EQ(*it, 3);
    EXPECT_EQ(genex::iterators::distance(vec.begin(), it), 6);
}
